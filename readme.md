...
* Amazon S3 One Zone-Infrequent Access 
    ...
    * **Objects must be stored at least 30 days in the current storage class before can be transitioned to STANDARD_IA or ONEZONE_IA**
...
- solution:
    ...
    - 3 types of Storage Gateway:
        - File Gateway
            ...
            - most recently used data is cachedin the File Gateway
        ...
        - Volume Gateway
            ...
            - cached volumes: entire dataset is stored on S3 and low latency access to most recent data
                - range from 1GB to 32 TB, can configure 32 volumes with a maximum storage of 1,024 TB (1 PB)
...
- AWS DataSync simplifies, automates, and accelerates moving data between on-premises storage systems and AWS Storage services, as well as between AWS Storage services.
    - AWS DataSync Agent is deployed as an agent on a server and connects to your NAS or file system to copy data to AWS and write data from AWS
...
- AWS Storage Gateway is more suitable to be used in integrating your storage services by replicating your data 
    - while AWS DataSync is better for workloads that require you to move or migrate your data.
...
* short polling vs long polling 
	...
	* with long polling, 
		* the ReceiveMessage request queries all of the servers for messages >> eliminate false empty responses
                * by setting the ReceiveMessageWaitTimeSeconds > 0
...
* overview:
    ...
    * as many `event receivers` (subscriptions) as we want to listen to the SNS topic notifications
		* notifications are **pushed** to subscribers via SMS, email, SQS to mobile devices or any HTTP endpoint
...
* CloudTrail events
    * management events:
        * provide visibility into **management operations** performed on or within a resource
    ...
    * data events:
        * provide visibility into **resource operations** performed on or within a resource
...
* CloudTrail event log files are encrypted using S3 server-side encryption (SSE)
    * you can also choose to encrypt with an AWS KMS key
...
* endpoints are virtual devices, scale horizontally and are redundant
...
* site-to-site VPN ECMR
    * ECMR = Equal-cost multi-path routing (need to be built on multiple VPC Tunnels)
...
* CloudFormation Building Blocks
    * templates components
        ...
        * outputs: references to what has been created / returned (optional section)
...
### AWS Systems Manager
* provides a unified user interface so you can track and resolve operational issues across AWS applications and resources (in AWS and on-prem) from a central place
* automate operational tasks for Amazon EC2 instances or Amazon RDS instances
* group resources by application,
* view operational data for monitoring and troubleshooting,
* implement pre-approved change work flows,
* and audit operational changes for your groups of resources. 
* `Run Command`, remotely and securely manage the configuration of your managed instances and automate common admin tasks and perform ad hoc configuration changes at scale
    * without having to login to each instance
    * no additional cost
